...
with DAG(
    dag_id='Lab_1_DAG',                               # Unique identifier for the DAG
    ...
    description='Daily stock data pipeline for AAPL and GOOGL using Snowflake and Yfinance'
) as dag:
    ...
    def fetch_stock_data():
        ...
        # api_key = Variable.get("VANTAGE_API_KEY")  # Retrieving API key from Airflow Variable
        ...
        for symbol in symbols:
            ...
            stock.columns = ['date', 'open', 'high', 'low', 'close', 'volume', 'symbol']
            all_data.append(stock)

        final_df = pd.concat(all_data, ignore_index=True)
        # final_df['date'] = final_df['date'].astype(str)  # Convert date to string
